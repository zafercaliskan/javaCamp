...
public interface ProductDao extends JpaRepository<Product, Integer> {
	...
	Product getByProductNameAndCategoryId(String productName, int categoryId); //findBy getBy aynıdır. İkiside olur. ProductName 1. kolon CategoryId 2. kolon. 
	//Bizim yerimize "and" operatörü olan where koşulu yazıyor.
	
	List<Product> getByProductNameOrCategoryId(String productName, int categoryId); //Burada hangi veri türünü döndürdüğümüz önemli. Yukarıda tek data getirirken burada birden fazla getirir.
	//"Or" operatörü olan where koşulu yazar. Yani 2 kolondan her hangi biriyle eşleşen koşul doğru olan alanları getirir.
	
	List<Product> getByCategoryIdIn(List<Integer> categories); //In birden fazla category id gönderip eşlesen tüm dataları getirebiliriz.
	
	List<Product> getByProductNameContains(String productName); 
	
	List<Product> getByProductNameStartWith(String productName); //Ürün isimleri verilen parametreye göre başlayan dataları getirir.
	
	List<Product> getByProductNameEndWith(String productName); //Ürün isimleri verilen parametreyle biten dataları getirir.
	
	//select * from products where category_id in(1, 2, 3, 4)
	
	@Query("From Product where productName=:productName and categoryId=:categoryId") //Burada veritabanında nasıl yazdığını unut. 
	...
	//select * from products where product_name=bişey and category_id=bişey -> bu sql sorgusu
}
